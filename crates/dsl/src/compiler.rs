...
mod tests {
    ...
    fn test_simple_emit() {
        ...
        "#;
        let parsed_ast = to_ast(snippet).expect("DSL parse failed for simple emit");
        let mut program = DatalogProgram::new();
        define_mermaid_relations(&mut program);
        define_solidity_relations(&mut program);
        traverse_dsl_ast(&parsed_ast, &mut program).expect("Compiling simple emit should succeed");
        let ddlog_str = format!("{}", program);
        let expected_ddlog = r#"input relation SequenceStmt(bigint, string);
input relation DiagramSequence_children(bigint, bigint);
input relation SolidityNode(bigint, string);
LangXOut([|Value from X: step1|]) :- [|Condition for step1|].
"#;
        assert_eq!(
            ddlog_str, expected_ddlog,
            "The emitted DDlog program does not match the expected output."
        );
    }

    // Test: Emit with Parent
    #[test]
    fn test_emit_with_parent() {
        let snippet = r#"
            (emit SolidityNode
                (SequenceStmt)@step1
                (parent Contract)
        ...
        "#;
        let parsed_ast = to_ast(snippet).expect("DSL parse failed for emit with parent");
        let mut program = DatalogProgram::new();
        define_mermaid_relations(&mut program);
        define_solidity_relations(&mut program);
        traverse_dsl_ast(&parsed_ast, &mut program)
            .expect("Compiling emit with parent should succeed");
        let ddlog_str = format!("{}", program);
        let expected_ddlog = r#"input relation SequenceStmt(bigint, string);
input relation DiagramSequence_children(bigint, bigint);
input relation SolidityNode(bigint, string);
input relation Contract(bigint);
input relation Contract_children(bigint, bigint);
LangXOut([|Value from X: step1|]) :- [|Condition for step1|].
Contract_children([|Parent condition for Contract|]) :- [|Condition for parent|].
"#;
        assert_eq!(
            ddlog_str, expected_ddlog,
            "The emitted DDlog program does not match the expected output."
    ...
    fn test_multiple_emits() {
        let snippet = r#"
            ...
            (emit Capture
                (SequenceStmt)@capture1
            )
        "#;
        let parsed_ast = to_ast(snippet).expect("DSL parse failed for multiple emits");
        let mut program = DatalogProgram::new();
        define_mermaid_relations(&mut program);
        define_solidity_relations(&mut program);
        traverse_dsl_ast(&parsed_ast, &mut program)
            .expect("Compiling multiple emits should succeed");
        let ddlog_str = format!("{}", program);
        let expected_ddlog = r#"input relation SequenceStmt(bigint, string);
input relation DiagramSequence_children(bigint, bigint);
input relation SolidityNode(bigint, string);
input relation Capture(bigint, string);
LangXOut([|Value from X: step1|]) :- [|Condition for step1|].
LangXOut([|Value from X: capture1|]) :- [|Condition for capture1|].
"#;
        assert_eq!(
            ddlog_str, expected_ddlog,
            "The emitted DDlog program does not match the expected output."
    ...
    // Test: Elaborate Transformation
    #[test]
    fn test_elaborate_transformation() {
        let snippet = r#"
            (emit SolidityNode
                (SequenceStmt)@init
                (emit Constructor
                    (SequenceStmt)@constructorInit
                    (capture @initData)
                )
                (emit FunctionDefinition
                    (SequenceStmt)@functionCall
                    (capture @callData)
                    (emit Modifier
                        (SequenceStmt)@modifierCheck
                    )
                )
            )
        "#;
        let parsed_ast = to_ast(snippet).expect("DSL parse failed for elaborate transformation");
        let mut program = DatalogProgram::new();
        define_mermaid_relations(&mut program);
        define_solidity_relations(&mut program);
        traverse_dsl_ast(&parsed_ast, &mut program)
            .expect("Compiling elaborate transformation should succeed");
        let ddlog_str = format!("{}", program);
        let expected_ddlog = r#"input relation SequenceStmt(bigint, string);
input relation DiagramSequence_children(bigint, bigint);
input relation SolidityNode(bigint, string);
input relation Constructor(bigint, string);
input relation Constructor_children(bigint, bigint);
input relation FunctionDefinition(bigint, string);
input relation FunctionDefinition_children(bigint, bigint);
input relation Modifier(bigint, string);
input relation Modifier_children(bigint, bigint);
input relation Capture(bigint, string);
LangXOut([|Value from X: init|]) :- [|Condition for init|].
LangXOut([|Value from X: constructorInit|]) :- [|Condition for constructorInit|].
Capture([|Value from X: initData|]) :- [|Condition for initData|].
LangXOut([|Value from X: functionCall|]) :- [|Condition for functionCall|].
Capture([|Value from X: callData|]) :- [|Condition for callData|].
LangXOut([|Value from X: modifierCheck|]) :- [|Condition for modifierCheck|].
"#;
        assert_eq!(
            ddlog_str, expected_ddlog,
            "The emitted DDlog program does not match the expected output."
    ...
    // Test: Emit without Captures or Parent
    #[test]
    fn test_emit_without_captures_or_parent() {
        let snippet = r#"
            (emit SolidityNode)
        "#;
        let parsed_ast =
            to_ast(snippet).expect("DSL parse failed for emit without captures or parent");
        let mut program = DatalogProgram::new();
        define_mermaid_relations(&mut program);
        define_solidity_relations(&mut program);
        traverse_dsl_ast(&parsed_ast, &mut program)
            .expect("Compiling emit without captures or parent should succeed");
        let ddlog_str = format!("{}", program);
        let expected_ddlog = r#"input relation SequenceStmt(bigint, string);
input relation DiagramSequence_children(bigint, bigint);
input relation SolidityNode(bigint, string);
