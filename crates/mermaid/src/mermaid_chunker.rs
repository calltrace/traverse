...
pub fn chunk_mermaid_diagram(diagram: &str, output_dir: Option<&Path>) -> Result<ChunkingResult> {
    ...
    for (chunk_id, boundary) in boundaries.iter().enumerate() {
        ...
        fs::write(&chunk_path, chunk_content).context(format!("writing chunk {}", chunk_num))?;
    ...
    fs::write(&index_path, index_content).context("writing index file")?;
    ...
    fs::write(&metadata_path, metadata_json).context("writing metadata file")?;
...
fn analyze_diagram(lines: &[String]) -> Result<DiagramAnalysis> {
    ...
    for line in lines {
        ...
        if trimmed.starts_with("participant ") {
            ...
            // Skip title lines - we don't use them in chunking logic
...
fn generate_chunk(
    ...
    if chunk_num > 1 {
        output.push(format!(
            "    Note over {}: Continued from Chunk {}",
            used_participants.first().unwrap_or(&"User".to_string()),
            chunk_num - 1
        ));
        output.push(String::new());
    }
...
fn generate_index(chunks: &[ChunkInfo], analysis: &DiagramAnalysis) -> String {
    ...
    output.push("sequenceDiagram".to_string());
    output.push(format!(
        "    title Mermaid Diagram Index - {} Chunks",
        chunks.len()
    ));
    output.push(String::new());
    ...
    output.push(String::new());
    output.push(format!(
        "    Note over Index: Total: {} lines<br/>{} participants<br/>{} interactions",
...
mod tests {
    ...
    fn test_small_diagram_no_chunking_needed() {
        ...
        assert_eq!(
            result.chunk_count, 1,
            "Small diagram should produce single chunk"
        );
    }
