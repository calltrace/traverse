...
./target/release/sol2cg contracts/*.sol -o callgraph.dot
...
- **Rust**: Version 1.83 or later ([Install Rust](https://rustup.rs/))
- **Git**: For cloning the repository with submodules

### Optional Dependencies

- **Foundry**: Required for `sol2test` and for project-level features ([Install Foundry](https://getfoundry.sh/))
- **Graphviz**: For visualizing DOT files generated by `sol2cg` ([Install Graphviz](https://graphviz.org/download/))
...
# Check Foundry (if needed)
forge --version

# Check Graphviz (if needed)
dot -V
```
...
### Method 2: Install via Cargo (when published)

```bash
# Install all traverse tools (when published to crates.io)
cargo install traverse-cli

# Or install from git repository directly
cargo install --git https://github.com/velvet-global/traverse

# Tools will be installed to ~/.cargo/bin/
```
...
# Run a specific tool
docker run --rm -v $(pwd):/workspace traverse:sol2cg /workspace/contracts
```

### Method 4: Pre-built Binaries

Download pre-built binaries from the [GitHub Releases](https://github.com/velvet-global/traverse/releases) page.
...
* Open an issue on [GitHub](https://github.com/velvet-global/traverse/issues)
...
For more information, visit the [Traverse GitHub repository](https://github.com/velvet-global/traverse).